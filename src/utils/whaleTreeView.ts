...
export function generateWhaleTreeView(): string {
  ...
  function renderNode(node: WhaleNode, level: number = 0): string {
    ...
    const getIcon = (type: string) => {
      switch (type) {
        case 'order':
          return 'icon-[tabler--crown]';
        case 'suborder':
          return 'icon-[tabler--git-branch]';
        case 'family':
          return 'icon-[tabler--users]';
        case 'genus':
          return 'icon-[tabler--dna]';
        case 'species':
          return `<svg class="w-5 h-5" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
    ...
    const getNodeUrl = (node: WhaleNode) => {
      switch (node.type) {
        ...
        case 'suborder':
          return `/suborder/${node.id}`;
        case 'family':
          return `/family/${node.id}`;
        case 'genus':
          return `/genus/${node.id}`;
        case 'species':
          return `/species/${node.id}`;
        default:
          return `#${node.id}`;
      }
    };
